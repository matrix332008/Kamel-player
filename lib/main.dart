...
class _TvLoginState extends State<TvLogin> {
  ...
  @override
  void dispose() {
    f1.dispose(); f2.dispose(); f3.dispose(); f4.dispose(); f5.dispose();
    super.dispose();
  }
  ...
  Widget btn(String t, FocusNode n, VoidCallback onTap, {bool selected = false, bool big = false}) {
    return Focus(
      focusNode: n,
      child: Builder(builder: (c) {
        ...
        return AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: selected ? Colors.pinkAccent : (t == 'اتصال' ? Colors.redAccent : Colors.indigo.shade900),
            borderRadius: BorderRadius.circular(30),
            border: has ? Border.all(color: Colors.white, width: 3) : null,
          ),
          child: InkWell(onTap: onTap, child: Center(child: Text(t, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)))),
        );
      }),
    );
  }

  Widget field(String h, TextEditingController c, FocusNode n, FocusNode next, TextInputType type, [bool p = false]) {
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          focusedBorder: const OutlineInputBorder(borderSide: BorderSide(color: Colors.pinkAccent, width: 2)),
        ),
      ),
    );
  }
            ...
            child: Column(
              ...
              children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  btn('Xtream Codes', f1, () => setState(() => isXtream = true), selected: isXtream),
                  const SizedBox(width: 20),
                  btn('M3U Playlist', f2, () => setState(() => isXtream = false), selected: !isXtream),
                ]),
                const SizedBox(height: 30),
                field('رابط السيرفر http://...', url, f3, f4, TextInputType.url),
                field('اسم المستخدم', user, f4, f5, TextInputType.text),
                field('كلمة المرور', pass, f5, f5, TextInputType.visiblePassword, true),
                const SizedBox(height: 25),
                btn('اتصال', f5, login, big: true),
              ],
            ),
          ),
        ),
  ...
  @override Widget build(BuildContext context){
    return Scaffold(
      body: Container(
        ...
        ListView.builder(padding: const EdgeInsets.all(40), itemCount: cats.length, itemBuilder: (_,i){
          final c = cats[i];
...
class _ChState extends State<ChannelsPage>{
  ...
  @override Widget build(BuildContext context){
    return Scaffold(
      ...
      GridView.builder(padding:const EdgeInsets.all(20), gridDelegate:const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:5,childAspectRatio:0.75), itemCount:ch.length, itemBuilder:(_,i){
        final s=ch[i];
        return Focus(autofocus:i==0, child: Builder(builder:(ctx){
          final f=Focus.of(ctx).hasFocus;
          return GestureDetector(onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>Player(url:s['stream_id'].toString(),host:widget.host,user:widget.user,pass:widget.pass,name:s['name']))),
