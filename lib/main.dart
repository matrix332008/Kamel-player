        ...
        return InkWell(
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Text(c.text.isEmpty? h : (obs? '••••••' : c.text), style: TextStyle(color: c.text.isEmpty? Colors.grey : Colors.black), overflow: TextOverflow.ellipsis)),
            ]),
          ),
        );
      }),
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadCats() async {
    if (type == 'm3u') { _loadStreams(); return; }
    ...
    if (type == 'm3u') {
      ...
      for (int i = 0; i < lines.length; i++) {
        if (lines[i].startsWith('#EXTINF')) {
          ...
          if (i + 1 < lines.length) s.add({'name': n, 'url': lines[i + 1].trim()});
        }
      }
        ...
        Container(
          ...
          child: Column(children: [
           ...
           ...List.generate(menus.length, (i) => Focus(
                  onFocusChange: (f) { if (f) setState(() => menu = i); },
                  child: Container(
        ...
        Container(
          ...
          child: loading? const Center(child: CircularProgressIndicator()) : ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return Focus(
                onFocusChange: (f) { if (f) setState(() { catId = c['category_id']; _loadStreams(); }); },
                child: Container(
                  color: sel? Colors.red.shade800 : Colors.transparent,
                  child: ListTile(title: Text(c['category_name']?? '', style: TextStyle(color: Colors.white, fontWeight: sel? FontWeight.bold : FontWeight.normal))),
                ),
              );
            },
          ),
          ...
          child: loading? const Center(child: CircularProgressIndicator()) : GridView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return Focus(
                autofocus: i == 0,
                onKey: (n, e) {
                  if (e is RawKeyDownEvent && e.logicalKey == LogicalKeyboardKey.select) {
                    final list = streams.map((e) => {'name': e['name']?? e['title']?? '', 'url': type == 'm3u'? e['url'] : _getUrl(e)}).toList();
                    Navigator.push(context, MaterialPageRoute(builder: (_) => PlayerPage(streams: list, index: i)));
                ...
                child: Builder(builder: (ctx) {
                  final has = Focus.of(ctx).hasFocus;
                  return Container(
                    decoration: BoxDecoration(color: Colors.grey[900], border: Border.all(color: has? Colors.red : Colors.transparent, width: 3), borderRadius: BorderRadius.circular(6)),
                    child: Column(children: [
                      Expanded(child: icon!= ''? Image.network(icon, fit: BoxFit.cover, width: double.infinity, errorBuilder: (_, __, ___) => const Icon(Icons.tv, color: Colors.white)) : const Icon(Icons.tv, color: Colors.white)),
                      Padding(padding: const EdgeInsets.all(4), child: Text(name, maxLines: 2, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 11))),
                    ]),
                  );
                }),
              );
...
class _PlayerPageState extends State<PlayerPage> {
  ...
  @override
  void initState() { super.initState(); idx = widget.index; _play(); }

  void _play() {
    ctrl = BetterPlayerController(
      const BetterPlayerConfiguration(autoPlay: true, fit: BoxFit.contain, controlsConfiguration: BetterPlayerControlsConfiguration(showControls: false)),
      betterPlayerDataSource: BetterPlayerDataSource(BetterPlayerDataSourceType.network, widget.streams[idx]['url']),
    );
    setState(() => showInfo = true);
    t?.cancel();
    t = Timer(const Duration(seconds: 2), () => setState(() => showInfo = false));
  }
  ...
  @override
  void dispose() { ctrl.dispose(); t?.cancel(); super.dispose(); }
        ...
        child: Stack(children: [
          ...
          AnimatedOpacity(
            ...
            child: Container(
              ...
              child: Stack(children: [
                Positioned(top: 20, right: 20, child: Text(_time(), style: const TextStyle(color: Colors.white, fontSize: 16))),
                Positioned(bottom: 30, left: 20, right: 20, child: Text(s['name'], style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold), textAlign: TextAlign.center)),
              ]),
            ),
          ),
        ]),
