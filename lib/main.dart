               ...
               ? const Center(child: CircularProgressIndicator())
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return Focus(
                        onFocusChange: (f) {
                          if (f) setState(() { catId = c['category_id']; _loadStreams(); });
                        },
                        child: Container(
                          color: sel? Colors.red.shade800 : Colors.transparent,
                          child: ListTile(
                            title: Text(c['category_name']?? '', style: TextStyle(color: Colors.white, fontWeight: sel? FontWeight.bold : FontWeight.normal)),
                          ),
                        ),
                      );
                    },
                        ...
                        child: Builder(builder: (context) {
                          ...
                          return Container(
                            ...
                            child: Column(
                              children: [
                                Expanded(
                                  child: icon!= ''? Image.network(icon, fit: BoxFit.cover, width: double.infinity, errorBuilder: (_, __, ___) => const Icon(Icons.tv, color: Colors.white)) : const Icon(Icons.tv, color: Colors.white),
                                ),
                                Padding(
                                  padding: const EdgeInsets.all(4),
                                  child: Text(name, maxLines: 2, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 11)),
                                ),
                              ],
                            ),
                          );
    ...
    showModalBottomSheet(
      ...
      builder: (_) => ListView.builder(
        ...
        itemBuilder: (_, i) => ListTile(
          title: Text(widget.streams[i]['name'], style: TextStyle(color: i == idx? Colors.red : Colors.white)),
          onTap: () { Navigator.pop(context); setState(() => idx = i); ctrl.dispose(); _play(); },
        ),
      ),
    );
  }
      ...
      body: Focus(
        ...
        onKey: (n, e) {
          if (e is RawKeyDownEvent) {
            ...
            if (e.logicalKey == LogicalKeyboardKey.select) { _showList(); return KeyEventResult.handled; }
