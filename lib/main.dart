  ...
  @override Widget build(BuildContext context){
    return Scaffold(body:Container(decoration:const BoxDecoration(image:DecorationImage(image:AssetImage('assets/background.jpeg'),fit:BoxFit.cover)),
      child:Center(child:SingleChildScrollView(padding:const EdgeInsets.all(20),child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        const SizedBox(height:8),const Text('Kamel TV',style:TextStyle(fontSize:26,color:Colors.red,fontWeight:FontWeight.bold)),const SizedBox(height:15),
        ...
        SizedBox(width:380,child:Column(children:[
          if(isXtream)...[
            whiteField(s,'رابط السيرفر',Icons.dns,false,true),const SizedBox(height:12),
            whiteField(u,'اسم المستخدم',Icons.person),const SizedBox(height:12),
            whiteField(p,'كلمة المرور',Icons.lock,true),const SizedBox(height:20),
            SizedBox(width:double.infinity,height:48,child:ElevatedButton(onPressed:saveX,style:ElevatedButton.styleFrom(backgroundColor:Colors.red),child:const Text('دخول',style:TextStyle(fontSize:20)))),
          ]else...[
            whiteField(m,'رابط M3U',Icons.link,false,true),const SizedBox(height:20),
            SizedBox(width:double.infinity,height:48,child:ElevatedButton(onPressed:saveM,style:ElevatedButton.styleFrom(backgroundColor:Colors.deepPurple),child:const Text('دخول',style:TextStyle(fontSize:20)))),
          ]
        ]))
      ])))));
  }

  // هذا الجديد - خانة بيضاء
  Widget whiteField(TextEditingController c,String h,IconData i,[bool obscure=false,bool auto=false]){
    return TextField(
      controller:c,
      obscureText:obscure,
      autofocus:auto,
